...
type
  TfmAluno = class(TForm)
    ...
    procedure edNota3KeyPress(Sender: TObject; var Key: Char);
...
begin
  try
    edNota1.Text := FormatFloat('0.00', StrToFloat(edNota1.Text));
...
begin
  try
    edNota2.Text := FormatFloat('0.00', StrToFloat(edNota2.Text));
...
begin
  try
    edNota3.Text := FormatFloat('0.00', StrToFloat(edNota3.Text));
...
procedure TfmAluno.edNota3KeyPress(Sender: TObject; var Key: Char);
begin
  if not(Key in ['0' .. '9', ',', #8]) then
  begin
    Key := #0;
  end;
  if (pos(',', edNota3.Text) > 0) and (Key = ',') then
  begin
    Key := #0;
  end;
end;
...
procedure TfmAluno.limpaInformacoes;
begin
  edMatricula.Clear;
  edNomeAluno.Clear;
